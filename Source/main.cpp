    ...
    while (!WindowShouldClose())
    {
        ...
        for (int i = 0; i < draw_queue.size(); ++i)
        {
            for (int j = 0; j < draw_queue[i]->components.size(); ++j)
            {
                ...
                plat::Animation_control *anim_control = draw_queue[i]->getComponent<plat::Animation_control>();

                if (anim_control)
                {
                    plat::Player_control *player_controler = draw_queue[i]->getComponent<plat::Player_control>();
                    plat::Transform *t = draw_queue[i]->getComponent<plat::Transform>();

                    //std::cout << t->pos.x << " " << t->pos.y << "\n";
                    //std::cout << player_controler->move_direction.x  << " " << player_controler->move_direction.y << "\n";

                    anim_control->Animation_caller(player_controler->move_direction, player_controler->attack_triger, {t->pos.x, t->pos.y});
                }
                if (spr)
                {
                    plat::Transform *t = draw_queue[i]->getComponent<plat::Transform>();
